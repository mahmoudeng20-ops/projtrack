...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _resetPassword() async {
    ...
    if (email.isEmpty || !_isValidEmail(email)) {
      _showSnackBar(
          isArabic
              ? "⚠️ يرجى كتابة بريدك الإلكتروني بشكل صحيح أولاً لإرسال رابط التعيين."
              : "⚠️ Please enter a valid email to send reset link.",
          Colors.red);
      return;
    }
    ...
    try {
      ...
      String message = isArabic
          ? "❌ تعذر إرسال البريد: ${e.message}"
          : "❌ Failed: ${e.message}";
      if (e.code == 'user-not-found') {
        message = isArabic
            ? "❌ هذا البريد الإلكتروني غير مسجل بالنظام."
  ...
  Future<void> _registerUser() async {
    ...
    try {
      ...
      if (uid != null) {
        ...
        _showSnackBar(
            isArabic
                ? "✅ تم تسجيل طلبك! بانتظار موافقة الإدارة لتفعيل الحساب."
                : "✅ Request registered! Awaiting admin approval to activate account.",
            Colors.orange);

        await _saveCredentials(_usernameController.text.trim(),
  ...
  Future<void> _loginUser() async {
    ...
    try {
      ...
      if (uid != null) {
        ...
        if (userDoc.exists) {
          ...
          if (status == 'approved') {
            ...
            await _saveCredentials(username, _emailController.text.trim(),
            ...
            _showSnackBar(
                isArabic
                    ? "⏳ عذراً، حسابك لا يزال قيد المراجعة والموافقة من الإدارة."
          ...
          _showSnackBar(
              isArabic
                  ? "❌ الحساب مسجل ولكن لم يتم العثور على بيانات الصلاحيات في Firestore."
                  : "❌ User authorization document not found in Firestore.",
              Colors.red);
          await FirebaseAuth.instance.signOut();
        }
      }
      ...
      if (e.code == 'user-not-found' ||
        ...
        message += isArabic
            ? "فشل الاتصال بالإنترنت. تحقق من الشبكة."
      ...
      _showSnackBar(isArabic ? "❌ خطأ غير متوقع: $e" : "❌ Unexpected Error: $e",
          Colors.red);
    } finally {
      if (mounted) setState(() => _isLoading = false);
    }
                    ...
                    children: [
                      ...
                      TextFormField(
                        ...
                        decoration: InputDecoration(
                          labelText: isArabic
                              ? "اسم المستخدم (مطلوب عند التسجيل الجديد)"
                              : "Username (Required for new registration)",
                      ...
                      OutlinedButton(
                        onPressed: () {
                          if (_usernameController.text.trim().isEmpty) {
                            _showSnackBar(
                                isArabic
                                    ? "⚠️ يرجى كتابة اسم المستخدم أولاً لإنشاء حساب جديد"
                                    : "⚠️ Please enter username first to register.",
                                Colors.red);
                            return;
                          }
                          _registerUser();
